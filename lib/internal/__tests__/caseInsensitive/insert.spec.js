    ...
    insert(entityId, newValue, createdAt, err => {
        ...
        batch
            ...
            .zscore(
                ...
                `${newValue.toLowerCase()}${MEMBER_SEPARATOR}${createdAt}${MEMBER_SEPARATOR}${entityId}${MEMBER_SEPARATOR}${newValue}`
            )
...
it('should enforce value uniqueness when differ by case', done => {
    const now = Date.now()
    async.series(
        [
            callback => {
