const { createEntityAttribute, UNIQUE } = require('./support')
...
const entityAttribute = createEntityAttribute(client, UNIQUE)
...
it('should be idempotent when two consecutive inserts, one entity, two values that only differ by case', done => {
    const entityId = uuidv4()
    const newValue = generate()
    let timestamp = Date.now()
...
it('should asynchronously fail when two consecutive inserts, two entities, two values that only differ by case', done => {
    const newValue = generate()
    let timestamp = Date.now()
