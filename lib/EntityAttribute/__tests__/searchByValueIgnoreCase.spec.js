...
it('should be defined', () => {
    expect(entityAttribute.searchByValueIgnoreCase).toBeDefined()
})

it('should be a function', () => {
    expect(typeof entityAttribute.searchByValueIgnoreCase).toBe('function')
})

it("should throw when argument 3, 'done' is not valid", () => {
    expect(() => {
        entityAttribute.searchByValueIgnoreCase()
...
it("should asynchronously fail when argument 1, 'value' is not valid", done => {
    const value = null
    const order = null
    entityAttribute.searchByValueIgnoreCase(value, order, err => {
...
it('should return an empty array', done => {
    ...
    entityAttribute.searchByValueIgnoreCase(value, order, (err, events) => {
        if (err) {
            done(err)
            return
        }
