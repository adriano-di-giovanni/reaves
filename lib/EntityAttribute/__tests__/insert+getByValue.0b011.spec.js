...
it.only('should return all events when two entities, the same null value', done => {
    const value = null
...
it('should return a single event when two entities, two non-null values that differ by case', done => {
    const value = generate()
    let timestamp = Date.now()
    const fixtures = [
        {
    ...
    async.mapSeries(fixtures, iteratee, err => {
        ...
        entityAttribute.getByValue(value, order, (err, events) => {
            ...
            expect(events).toEqual([fixtures[0]])
