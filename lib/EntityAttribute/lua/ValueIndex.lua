local ValueIndex = {}
ValueIndex.__index = ValueIndex

function ValueIndex:create(keyPrefix, isCaseSensitive, memberSeparator)
    local instance = {
        key = keyPrefix .. ':ValueIndex',
        isCaseSensitive = isCaseSensitive,
        memberSeparator = memberSeparator
    }
    setmetatable(instance, ValueIndex)
    return instance
end

function ValueIndex:buildMember(entityId, value, createdAt)
    local memberSeparator = self.memberSeparator

    if (self.isCaseSensitive) then
        return value .. memberSeparator .. createdAt .. memberSeparator .. entityId
    end

    return string.lower(value) ..
        memberSeparator .. createdAt .. memberSeparator .. entityId .. memberSeparator .. value
end

function ValueIndex:delete(entityId, value, createdAt)
    local member = self:buildMember(entityId, value, createdAt)
    return redis.call('ZREM', self.key, member)
end

function ValueIndex:getMostRecent(value)
    local query = value
    local isTerm = true
    local order = ORDER_DESCENDING
    local offset = 0
    local count = 1
    local reply = self:searchRaw(query, isTerm, order, offset, count)
    local member = reply[1]
    local entityId = nil
    local createdAt = nil

    value = nil

    if (member ~= nil) then
        entityId, value, createdAt = self:unbuildMember(member)
    end

    return entityId, value, createdAt
end

function ValueIndex:searchRaw(aQuery, isTerm, order, offset, count)
    local query

    if (self.isCaseSensitive) then
        query = aQuery
    else
        query = string.lower(aQuery)
    end

    if (isTerm) then
        query = query .. self.memberSeparator
    end

    local command
    local min
    local max
    if (order == ORDER_ASCENDING) then
        command = 'ZRANGEBYLEX'
        min = '[' .. query
        max = '[' .. query .. TRAILING_BYTE
    else
        command = 'ZREVRANGEBYLEX'
        min = '[' .. query .. TRAILING_BYTE
        max = '[' .. query
    end

    if (offset ~= nil and count ~= nil) then
        return redis.call(command, self.key, min, max, 'LIMIT', offset, count)
    end

    return redis.call(command, self.key, min, max)
end

function ValueIndex:insert(entityId, value, createdAt)
    local score = 0
    local member = self:buildMember(entityId, value, createdAt)
    return redis.call('ZADD', self.key, score, member)
end

function ValueIndex:unbuildMember(member)
    local memberSeparator = self.memberSeparator

    local pattern
    local entityId
    local value
    local createdAt

    if (self.isCaseSensitive) then
        pattern = '^(.+)' .. memberSeparator .. '(%d+)' .. memberSeparator .. '(.+)$'
        value, createdAt, entityId = string.match(member, pattern)
    else
        pattern = '^(.+)' ..
            memberSeparator .. '(%d+)' .. memberSeparator .. '(.+)' .. memberSeparator .. '(.+)$'
        local _
        _, createdAt, entityId, value = string.match(member, pattern)
    end

    return entityId, value, tonumber(createdAt, 10)
end
